...
class Annotator():
    ...
    def draw_direction(self):
        ...
        p1 = self.nose
        nose_xy.append(p1)
        cv2.circle(self.im,p1,3,Color.green,-3)
    ...
    def avg():
        x=0
        y=0
        for i in range(len(nose_xy)):
            x+=nose_xy[i][0]
        for j in range(len(nose_xy)):
            y+=nose_xy[j][1]
        x=x/len(nose_xy)
        y=y/len(nose_xy)
        print("x_coordinate_avg for nose is: {} and y_coordinate_avg for nose is: {}".format(x,y))
